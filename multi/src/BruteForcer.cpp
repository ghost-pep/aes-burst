...
void BruteForcer::brute_force(vector<string> *keys, vector<string> *samples) {
	...
	for (auto key_it = keys->begin(); key_it != keys->end(); ++key_it) {
		...
		pool->push( //lambda below that captures the key and sample vectors
			[key, ciphertexts_cpy, one_block_ciphertexts_cpy, multi_block_ciphertexts_cpy, mode_cpy, this] (int id) {
				...
				if (mode_cpy == ECB || mode_cpy == ALL) {
					...
					test_cbc(decoded_key, one_block_ciphertexts_cpy, multi_block_ciphertexts_cpy);
...
void BruteForcer::test_cbc(string key,
	...
	if (multi_block_ciphertexts->size() > 0) {
		for (auto str_it = multi_block_ciphertexts->begin();
			...
			if (crib.empty()) {
				...
				string makeshift_iv = whole_cipher.substr(
						whole_cipher.size() - 32, CryptoPP::AES::BLOCKSIZE
						);
				
				//populate ciphertext with the last block
				string ciphertext = whole_cipher.substr(
						whole_cipher.size() - 16, CryptoPP::AES::BLOCKSIZE
						);
	...
	if (one_block_ciphertexts->size() > 0) {
		if (!iv.empty()) {
			...
			for (auto str_it = one_block_ciphertexts->begin();
				...
				CryptoPP::CBC_Mode<CryptoPP::AES>::Decryption d;
				d.SetKeyWithIV((CryptoPP::byte *) key.data(),
						key.size(),
						(CryptoPP::byte *) iv.data(),
						iv.size()
						);
...
bool BruteForcer::decrypt_and_check(CryptoPP::SymmetricCipher *cipher, string ciphertext) {
	...
	if (crib_len != 0) {
		try {
			...
			//decrypt
			CryptoPP::StringSource cribsource(ciphertext, true,
			...
			/* CryptoPP::StreamTransformationFilter stfDecryptor_crib(*cipher, &cribsink, CryptoPP::BlockPaddingSchemeDef::BlockPaddingScheme::NO_PADDING); */
			/* cout << "CIPHERTEXT: "; */
